...
export interface AmapNaviPlugin {
     ...
     * 需要在启动导航前调用一次
     */
    initialize(config: NaviConfig): Promise<{
        success: boolean;
    }>;
    /**
     ...
     */
    startEmbeddedNavi(options: NaviLaunchOptions): Promise<{
        success: boolean;
    }>;
    /**
     ...
     */
    launchNavi(options: NaviLaunchOptions): Promise<{
        success: boolean;
    }>;
    /**
     * 判断高德导航 App 是否已安装
     */
    isAmapInstalled(): Promise<{
        installed: boolean;
    }>;
    /**
     * 获取当前 GPS 位置（一次性）
     ...
     */
    startLocationTracking(callback: (location: {
        latitude: number;
        longitude: number;
        bearing: number;
        speed: number;
     ...
     * 计算两点间距离（米）
     */
    calculateDistance(from: NaviRoutePoint, to: NaviRoutePoint): Promise<{
        distance: number;
    }>;
    /** 导航开始事件 */
    ...
    /** 到达目的地事件 */
    addListener(eventName: 'arrived', listener: (data: {
        type: 'waypoint' | 'destination';
        index: number;
    }) => void): Promise<PluginListenerHandle>;
    /** 位置更新事件（startLocationTracking 时触发） */
    addListener(eventName: 'locationUpdate', listener: (data: {
        latitude: number;
        longitude: number;
        bearing: number;
        speed: number;
